...
export class Weapon {
    constructor(camera, scene, particles) {
        this.camera = camera;
        this.scene = scene;
        this.particles = particles; // 注入粒子对象池
        ...
        this.maxAmmo = 120;
        this.lastFireTime = 0;
        this.fireRate = 100; // 射速限制：100ms 一发
    ...
    fire(raycaster) {
        ...
        this.ammo--;
        this.recoil = 0.12; // 屏幕后坐力反馈
        this.flash.intensity = 15;

        // 更新 UI
        document.querySelector('.ammo').innerText = `${this.ammo} / ${this.maxAmmo}`;

        // 射线检测
        raycaster.setFromCamera(new THREE.Vector2(0, 0), this.camera);
        // 仅检测 Layer 0 (场景和敌人)，忽略 Layer 1 (武器自身)
        ...
        if (hits.length > 0) {
            ...
            // 检查是否命中敌人 (依赖上一阶段 Enemy.js 注入的 userData)
            if (object.userData && object.userData.isEnemy) {
                const enemyRef = object.userData.ref;
                // 计算击退方向向量 (仅取水平面)
                const hitDir = new THREE.Vector3().subVectors(hit.point, this.camera.position).normalize();
                hitDir.y = 0; 
                enemyRef.takeDamage(25, hitDir);
                
                // 击中敌人产生红色火星 (也可以额外加个血效)
                this.particles.spawnImpact(hit.point, hit.face ? hit.face.normal : new THREE.Vector3(0,1,0));
            } else {
                // 击中环境，生成火花，法线用于粒子反弹方向
                const normal = hit.face ? hit.face.normal.clone().transformDirection(object.matrixWorld) : new THREE.Vector3(0, 1, 0);
                this.particles.spawnImpact(hit.point, normal);
            }
        }
    }
}